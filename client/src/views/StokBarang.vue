...
function emptyForm() {
  return { sku: '', name: '', category: 'Laptop', brand: 'Apple', purchase_price: 0, sell_price: 0, stock_qty: 0, min_stock: 3, unit: 'unit' };
}
  ...
  <div class="pt-2">
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-sm text-neutral-500">Kelola produk dan pantau ketersediaan stok toko.</p>
      </div>
      <button v-if="canEdit" class="btn-primary" @click="openCreate"><Plus :size="16" /> Tambah Produk</button>
    </div>
    ...
    <div class="card p-4 mb-4 flex flex-wrap gap-3">
      <div class="relative flex-1 min-w-[220px]">
        ...
        <input v-model="search" @input="onSearchInput" class="input !pl-9" placeholder="Cari nama atau SKU produk..." />
      </div>
    ...
    <div class="card p-5">
      <EmptyState v-if="!loading && !products.length" message="Belum ada produk." />
    ...
    <Modal v-if="showModal" :title="editing ? 'Edit Produk' : 'Tambah Produk'" @close="showModal = false">
      <form @submit.prevent="submit" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <input v-model="form.category" class="input" placeholder="Laptop / Aksesoris / Tablet" />
          </div>
          <div>
