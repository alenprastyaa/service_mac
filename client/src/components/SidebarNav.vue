...
<template>
  <aside
    class="w-64 shrink-0 bg-white dark:bg-neutral-900 text-neutral-600 dark:text-neutral-300 flex flex-col h-screen border-r border-neutral-200 dark:border-neutral-800 fixed inset-y-0 left-0 z-40 transition-transform duration-200 lg:sticky lg:top-0 lg:translate-x-0"
  ...
  >
    <div class="px-5 py-5 flex items-center justify-between">
      ...
      <button class="lg:hidden w-9 h-9 rounded-lg flex items-center justify-center text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 shrink-0" @click="ui.closeSidebar()">
        <X :size="18" />
      </button>
    </div>
    ...
    <nav class="flex-1 px-3 space-y-1 overflow-y-auto">
      <router-link
        ...
        :class="isActive(item.to) ? 'bg-gradient-to-br from-brand-400 to-brand-600 text-white shadow-lg shadow-brand-500/30' : 'text-neutral-600 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-800 hover:text-ink-900 dark:hover:text-white'"
      >
